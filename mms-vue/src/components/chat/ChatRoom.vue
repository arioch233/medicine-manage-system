  ...
  <div>
    ...
    >
      ...
      <div class="header">
        ...
        <i class="close" @click="isShow = false">
          mdi-close
        </i>
      </div>
      <!-- 对话内容 -->
      <div class="message" id="message">
        ...
        >
          <i class="iconfont  icon-yuyin  close-voice"/>
        </div>
        <div
      ...
      <div class="footer">
        ...
        />
        <i
            class="iconfont icon-jianpan-xianxing"
            v-show="isVoice"
            @click="isVoice = !isVoice"
            style="margin-right: 8px;font-size: 1.5rem"
        />
...
export default {
  ...
  methods: {
    ...
    connect() {
      const that = this;
      this.websocket = new WebSocket(this.website.websocketUrl);
      // 连接发生错误的回调方法
  ...
  computed: {
    isSelf() {
      return function (item) {
        return (
            (item.userId != null && item.userId === this.$store.state.userId)
        );
      };
    },
    isleft() {
    ...
    avatar() {
      ...
      return this.$store.state.avatar != null
          ...
          : this.$store.state.webSite.userAvatar;
    },
    userId() {
    ...
    isInput() {
      return this.content.trim() !== ""
          ? "iconfont icon-paper-full submit-btn"
          : "iconfont icon-paper-full";
    }
  }
};
</script>
