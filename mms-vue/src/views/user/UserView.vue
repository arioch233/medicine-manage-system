<template>
  <el-card class="main-card">
    ...
    <el-dialog :visible.sync="isAdd" width="30%">
      ...
      <el-form label-width="60px" size="medium" :model="addUserAccountForm">
        <el-form-item label="邮箱">
          <el-input v-model="addUserAccountForm.username" style="width:220px"/>
        </el-form-item>
        <el-form-item label="昵称">
          <el-input v-model="addUserAccountForm.nickname" style="width:220px"/>
        </el-form-item>
        <el-form-item label="角色">
          <el-checkbox-group v-model="roleIdList">
            <el-checkbox
        ...
        <span style="margin-left: 20px;color: red;font-size: 0.25rem">初始密码: 123456</span>
      </el-form>
      <div slot="footer">
        <el-button @click="isAdd = false">取 消</el-button>
        <el-button type="primary" @click="addUserAccount">
          确 定
        </el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    changeDisable(user) {
      this.request.put("/users/disable", {
        id: user.userInfoId,
    ...
    addUserAccount() {
      this.addUserAccountForm.roleId = this.roleIdList;
      this.request.post("/user/add", this.addUserAccountForm).then(data => {
        if (data.code === 200) {
          this.$notify.success({
            title: "成功",
            message: data.message
    ...
    listRoles() {
      this.request.get("/role/options").then(data => {
